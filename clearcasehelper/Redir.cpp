...
HWND CRedirector::m_hWnd = 0; 
UINT CRedirector::m_msg = 0;
UINT CRedirector::m_param = 0;
...
BOOL CRedirector::Open(LPCTSTR pszCmdLine)
{
	...
	__finally
	{
		if (!bOK)
		{
			...
			TCHAR szMsg[40];
			::sprintf(szMsg, _T("Redirect console error: %x\r\n"), dwOsErr);
			WriteStdError(szMsg);
			DestroyHandle(hStdoutReadTmp);
			DestroyHandle(hStdoutWrite);
			DestroyHandle(hStderrWrite);
...
DWORD WINAPI CRedirector::OutputThread(LPVOID lpvThreadParam)
{
    ...
    if(m_hWnd && m_msg){
        ::PostMessage(m_hWnd, m_msg, m_param, 0);
    }
